...
public final class StaticMessage implements IMessage {
  private static final @NonNull CompletableFuture<@Nullable Void> FINISHED = CompletableFuture.completedFuture(null);
  ...
  public @NonNull CompletableFuture<@Nullable Void> send(final @NonNull Audience audience) {
    audience.sendMessage(MiniMessage.get().parse(this.messageKey.map(this.messagesConfig)));
    return FINISHED;
  }
}
